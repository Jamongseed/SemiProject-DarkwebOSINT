...
st.set_page_config(page_title="Darkweb Leak Dashboard", layout="wide")
st.title("Darkweb Leak Dashboard")
...
uploaded = st.file_uploader("분석할 CSV 파일을 업로드하세요", type=["csv"], accept_multiple_files=True)
...
if uploaded:
    ...
    st.info("이전에 업로드하신 CSV를 복원했습니다. 필요하면 위에서 새 파일을 업로드하여 교체하실 수 있습니다.")
...
ISO2_TO_ISO3 = {"US":"USA","GB":"GBR","UK":"GBR","KR":"KOR","RU":"RUS","CN":"CHN","JP":"JPN","DE":"DEU","FR":"FRA","IT":"ITA","ES":"ESP","NL":"NLD","AE":"ARE","SA":"SAU","KSA":"SAU","BR":"BRA","IN":"IND","ID":"IDN","VN":"VNM","CA":"CAN","AU":"AUS","TR":"TUR","MX":"MEX","SE":"SWE","CH":"CHE","PL":"POL","TW":"TWN","HK":"HKG","SG":"SGP","TH":"THA","PH":"PHL","IR":"IRN","IQ":"IRQ","IL":"ISR","UA":"UKR","AR":"ARG","CO":"COL","ZA":"ZAF","DK":"DNK","NO":"NOR","FI":"FIN","BE":"BEL","PT":"PRT","IE":"IRL","RO":"ROU","HU":"HUN","CZ":"CZE","SK":"SVK"}
ALIASES = {"united states":"USA","united states of america":"USA","america":"USA","united kingdom":"GBR","great britain":"GBR","england":"GBR","south korea":"KOR","korea":"KOR","republic of korea":"KOR","uae":"ARE","russia":"RUS","russian federation":"RUS","saudi arabia":"SAU","ksa":"SAU","hong kong":"HKG","hongkong":"HKG"}
...
EXCLUDE_DOMAINS = {"mega.nz","anonfiles.com","gofile.io","krakenfiles.com","mediafire.com","transfer.sh","file.io","drive.google.com","docs.google.com","dropbox.com","onedrive.live.com","googleusercontent.com","pastebin.com","ghostbin.com","rentry.co","hastebin.com","t.me","telegram.me","telegram.org","x.com","twitter.com","facebook.com","linkedin.com","github.com","gitlab.com","breachforums.st","breachforums.is","breachforums.vc","breachforums.cx","xss.is","raidforums.com","dragonforce.io","dragonforceasia.org","dragonforce.my","dragonforce.pk","ransomware.live","ransomwatch.telemetry.ltd"}
...
def wikidata_country_iso3_for_company(company_name: str) -> str|None:
    ...
    if not props or "query" not in props:
        return None
...
def google_country_iso3_by_places_multi(company: str, api_key: str) -> str|None:
    if not company:
        return None
    variants = [company, f"{company} headquarters", f"{company} company", f"{company} government", f"{company} official"]
    total = {}
    for q in variants:
...
def build_text(row):
    parts = []
    for c in ["record_type","description","content","title","hashtags","notes","source","actor","ransomware_group","company","details_url","website","attachment_urls"]:
        if c in row and pd.notna(row[c]):
            parts.append(str(row[c]))
    return " | ".join(parts).lower()

def parse_any_dt(row):
    for c in ["posted_at_utc","post_date_utc","post_datetime_utc","published_at_utc","crawled_at_utc","crawled_at_kst","discovery_date","reported_at_utc"]:
        if c in row and pd.notna(row[c]):
...
df["__ts"] = df.apply(parse_any_dt, axis=1)
df["__size_gib"] = df.apply(get_size_gib, axis=1)
df["__company_norm"] = df["company"].map(norm_company) if "company" in df.columns else ""

HEUR_PATTERNS = [(r"\bS\.p\.A\.?\b","ITA",1.0),(r"\bs\.r\.l\.?\b","ITA",0.8),(r"\bGmbH\b","DEU",1.0),(r"\bAG\b","CHE",0.6),(r"\bOy\b","FIN",1.0),(r"\bAB\b","SWE",0.8),(r"\bSp\. z o\.o\.\b","POL",1.0),(r"\bs\.r\.o\.\b","CZE",0.9),(r"\bd\.o\.o\.\b","SVN",0.7),(r"\bK\.K\.?\b","JPN",1.0),(r"\bS\.A\. de C\.V\.\b","MEX",1.0),(r"\bA/S\b","DNK",1.0),(r"\bAS\b","NOR",0.6),(r"\bPvt\.?\s+Ltd\.?\b","IND",1.0),(r"\bLtd\.?\b","GBR",0.6),(r"\bLLC\b","USA",0.5),(r"\bInc\.?\b","USA",0.5),(r"\bSAS\b","FRA",0.6),(r"\bS\.A\.?\b","ESP",0.4)]
...
def heuristic_country_iso3(company: str, text: str, hosts: set[str]) -> str|None:
    ...
    for pat, iso3, w in HEUR_PATTERNS:
        if re.search(pat, blob, re.IGNORECASE):
            votes[iso3] = votes.get(iso3, 0.0) + w
    SLD_TO_ISO2 = {"com.au":"AU","net.au":"AU","org.au":"AU","co.kr":"KR","or.kr":"KR","go.kr":"KR","ac.kr":"KR","com.br":"BR","com.tr":"TR","com.mx":"MX","com.ar":"AR","com.pe":"PE","com.co":"CO","co.jp":"JP","ne.jp":"JP","or.jp":"JP","ac.jp":"JP","go.jp":"JP","co.uk":"GB","ac.uk":"GB","gov.uk":"GB","org.uk":"GB","ltd.uk":"GB","com.sg":"SG","com.my":"MY","com.hk":"HK","com.tw":"TW","com.sa":"SA","com.pl":"PL","com.ru":"RU","com.ua":"UA","com.pt":"PT","com.ro":"RO","com.cz":"CZ","com.vn":"VN","com.ph":"PH","com.id":"ID","co.za":"ZA","com.ng":"NG","com.eg":"EG"}
    for h in hosts:
...
SENS_PATS = [("wallet_keys", r"\b(private key|seed phrase|mnemonic|wallet\.dat|api key|jwt|ssh key)\b", 50),("pii", r"\b(personal data|pii|ssn|passport|national id|주민등록|여권|운전면허)\b", 40),("financial", r"\b(credit card|iban|bank|송금|계좌|financial)\b", 40),("credentials", r"\b(credentials?|passwords?|hash(?:es)?|combo(?:list)?|stealer logs?|cookies?)\b", 35),("db_dump", r"\b(database|db dump|sql dump|backup|mongodb|postgres|mysql)\b", 35),("source_code", r"\b(source code|git leak|repository)\b", 25),("access_infra", r"\b(vpn|rdp|citrix|okta|admin panel|zimbra|o365)\b", 25),("lists", r"\b(email lists?|phone lists?|dox|fullz)\b", 20)]
RANSOM = {"lockbit","blackcat","alphv","play","cl0p","medusa","black basta","akira","8base","bianlian","cactus","ragroup","cuba","royal","conti","ransomh0use","anubis"}
...
def exp_stage(t, row):
    ...
    if isp or re.search(r"\b(leaked|published|dumped|released)\b", t):
        return "published"
    if re.search(r"\b(for sale|selling|price|btc|xmr|monero|bitcoin)\b", t):
        return "for_sale"
...
def exp_score(stage):
    return {"published":30, "for_sale":20, "announced":15, "listed":10}.get(stage, 10)

def actor_score(t):
    s = 0
    if any(a in t for a in RANSOM):
        s = max(s, 20)
    if any(a in t for a in HACKT):
        s = max(s, 12)
    if any(a in t for a in RESELL):
        s = max(s, 8)
    return s
...
def mentions_score(t):
    s = 0
    if re.search(r"\b(admin|root|privileged|domain admin|global admin)\b", t):
        s += 10
    if re.search(r"\b(okta|adfs|azure ad|o365|exchange)\b", t):
        s += 5
    if re.search(r"\b(ransom|btc|xmr|monero|bitcoin|demand)\b", t):
        s += 8
    return min(s, 23)
...
def cross_score(norm):
    return 10 if norm and norm in cross_keys else 0
...
st.sidebar.caption(f"합계: {sum(weights_raw.values())} → 내부 환산 100 기준")

w_df = pd.DataFrame({"component": list(norm_w.keys()), "weight": list(norm_w.values())})
pie = alt.Chart(w_df).mark_arc(innerRadius=60).encode(theta="weight:Q", color="component:N", tooltip=["component","weight"])
...
for k in NMAX:
    raw[f"n_{k}"] = raw[k].apply(lambda v: to_norm(v, NMAX[k]))
...
if enrich_method.startswith("Google"):
    gmaps_api_key = st.sidebar.text_input("Google Maps API Key", type="password", help="Places Text Search + 국가 추정")
...
by_src = (v.assign(sev=v["severity_level"].astype("category").cat.set_categories(order_levels, ordered=True)).groupby(["__dataset","sev"], observed=False).size().reset_index(name="count"))
chart1 = (alt.Chart(by_src).mark_bar().encode(y=alt.Y("__dataset:N", title="Source", sort='-x', axis=alt.Axis(labelAngle=0)), x=alt.X("count:Q", title="Count", stack="zero"), color=alt.Color("sev:N", title="Severity", scale=alt.Scale(domain=order_levels)), tooltip=["__dataset","sev","count"]).properties(height=320))
...
by_bin = (v.groupby(["__dataset","score_bin"], observed=False).size().reset_index(name="count"))
chart2 = (alt.Chart(by_bin).mark_bar().encode(y=alt.Y("__dataset:N", title="Source", sort='-x', axis=alt.Axis(labelAngle=0)), x=alt.X("count:Q", title="Count", stack="zero"), color=alt.Color("score_bin:N", title="Score Bucket", sort=labels), tooltip=["__dataset","score_bin","count"]).properties(height=320))
altair_chart(st, chart2)

st.subheader("피해 국가 현황")
if HAS_PLOTLY and v["victim_country_iso3"].notna().any():
    geo = (v.dropna(subset=["victim_country_iso3"]).groupby("victim_country_iso3").size().reset_index(name="count").sort_values("count", ascending=False))
    ISO3_TO_KO = {"KOR":"대한민국","USA":"미국","GBR":"영국","FRA":"프랑스","DEU":"독일","JPN":"일본","CHN":"중국","RUS":"러시아","CAN":"캐나다","AUS":"호주","ITA":"이탈리아","ESP":"스페인","NLD":"네덜란드","SWE":"스웨덴","NOR":"노르웨이","DNK":"덴마크","FIN":"핀란드","CHE":"스위스","POL":"폴란드","PRT":"포르투갈","IRL":"아일랜드","BEL":"벨기에","CZE":"체코","SVK":"슬로바키아","HUN":"헝가리","ROU":"루마니아","AUT":"오스트리아","GRC":"그리스","TUR":"튀르키예","UKR":"우크라이나","ISR":"이스라엘","SAU":"사우디아라비아","ARE":"아랍에미리트","IND":"인도","IDN":"인도네시아","VNM":"베트남","THA":"태국","PHL":"필리핀","SGP":"싱가포르","MYS":"말레이시아","HKG":"홍콩","TWN":"대만","MEX":"멕시코","ARG":"아르헨티나","BRA":"브라질","COL":"콜롬비아","CHL":"칠레","PER":"페루","ZAF":"남아프리카공화국","EGY":"이집트","NGA":"나이지리아"}
    geo["Country"] = geo["victim_country_iso3"].map(ISO3_TO_KO).fillna(geo["victim_country_iso3"])
    fig_choro = px.choropleth(geo, locations="victim_country_iso3", locationmode="ISO-3", color="count", color_continuous_scale="Reds", title="유출 대상 회사의 위치 기준", labels={"count":"유출 건수"})
    fig_choro.update_traces(customdata=np.stack([geo["Country"]], axis=-1), hovertemplate="Country=%{customdata[0]}<br>건수=%{z}<extra></extra>")
...
st.subheader("상세")
show_cols = ["__dataset","company","country","victim_country_iso3","__ts","__size_gib","exposure_stage","severity_level","severity_score"]
show_cols = [c for c in show_cols if c in v.columns]
dataframe_(st, v.sort_values("severity_score", ascending=False)[show_cols])

st.download_button("💾 현재 뷰 CSV 다운로드", v.to_csv(index=False).encode("utf-8"), "dashboard_export.csv", "text/csv")
